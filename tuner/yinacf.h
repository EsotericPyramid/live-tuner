...
public:
    ...
    */
    ...
    bool build (unsigned windowSize, unsigned tmax) {
        ...
        for (unsigned i = 0; i < TMAX; ++i)
            sol[i] = Solution(W); // modified from original by EsotericPyramid to avoid directly calling constructor
    ...
    */
    ...
    inline const Sample getFrequency(int offset = 0) const { //type manually added by EsotericPyramid, not sure why it wasn't there
...
private:
    ...
    //template<class T> //template removed by EsotericPyramid
    inline static void computeDiff(YinACF<Sample>* that, const Sample& s, typename YinACF<Sample>::Solution* pscur, typename YinACF<Sample>::Solution* psprev) {
        Sample sum = 1;
        Sample ii = 1;
